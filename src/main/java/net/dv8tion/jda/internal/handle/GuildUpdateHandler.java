...
public class GuildUpdateHandler extends SocketHandler {
    ...
    protected Long handleInternally(DataObject content) {
        ...
        if (!Objects.equals(bannerId, guild.getBannerId())) {
            ...
            getJDA().handleEvent(
                            new GuildUpdateBannerEvent(getJDA(), responseNumber, guild, oldBanner));
        }
        ...
        if (!Objects.equals(name, guild.getName())) {
            ...
            guild.setName(name);
            getJDA().handleEvent(
                            new GuildUpdateNameEvent(getJDA(), responseNumber, guild, oldName));
        }
        if (!Objects.equals(iconId, guild.getIconId())) {
            String oldIconId = guild.getIconId();
            guild.setIconId(iconId);
            getJDA().handleEvent(
                            new GuildUpdateIconEvent(getJDA(), responseNumber, guild, oldIconId));
        }
        ...
        if (!Objects.equals(locale, guild.getLocale())) {
            ...
            getJDA().handleEvent(
                            new GuildUpdateLocaleEvent(getJDA(), responseNumber, guild, oldLocale));
        }
